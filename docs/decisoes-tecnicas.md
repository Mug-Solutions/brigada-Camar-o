...
## 2026-09-14 — Delay perceptível trocando de menu: Custom Access Token Hook tira uma consulta por navegação

**121. Cliente reportou delay em toda troca de menu.** Causa: `src/proxy.ts` (roda em toda navegação pra rota protegida) fazia DUAS chamadas de rede em sequência — `supabase.auth.getUser()` (validação de sessão, obrigatória) e depois uma consulta em `usuarios` só pra saber papel/ativo. A segunda é sempre a mesma pergunta ("esse usuário é staff ativo? bombeiro ativo?"), repetida a cada clique.

**122. Corrigido com um Custom Access Token Hook (`public.custom_access_token_hook`, migração `0022_auth_hook_papel_no_token.sql`) que embute `papel`/`ativo` direto em `app_metadata` do JWT toda vez que o token é emitido/renovado.** `proxy.ts` trocou `getUser()` + query por `getClaims()` (decodifica/verifica o próprio JWT) — a segunda ida ao banco deixou de existir. Precisou de 3 passos manuais no painel do Supabase, nenhum automatizável por código: (a) rodar a migração no SQL Editor, (b) criar o Auth Hook "Customize Access Token (JWT) Claims" apontando pra essa função, (c) ambos concluídos e confirmados antes do deploy.

**123. Trade-off aceito conscientemente: revogação deixou de ser instantânea.** Antes, desativar/recusar alguém cortava o acesso na próxima navegação (query sempre fresca). Agora `papel`/`ativo` só atualizam quando o token renova — por isso o "Access token expiry time" (Authentication → Sessions → Access Tokens) foi encurtado de `3600` pra `600` segundos, limitando essa janela a no máximo 10 min em vez de 1h. Não confundir com "Time-box user sessions"/"Inactivity timeout" (mesma tela, mas bloqueado no plano Pro) — é um campo separado, disponível em qualquer plano.

**124. Revisão de segurança sem CRITICAL/HIGH.** Validação de `app_metadata` é fail-closed em todos os ramos degenerados (ausente, tipo errado, papel fora de `PAPEIS`) — nunca libera acesso por omissão. Confirmado lendo o código-fonte do `@supabase/auth-js` que, com chave de assinatura **simétrica** (a que este projeto usa), `getClaims()` sempre faz uma chamada real ao servidor de Auth por baixo dos panos — um JWT adulterado ainda é pego na hora, não é confiança cega no payload decodificado localmente. Achado MEDIUM só relevante se o projeto um dia migrar pra chave assimétrica (aí a verificação passa a ser local via JWKS cacheado, e a revogação de sessão deixa de ser detectada em tempo real — não só papel/ativo). Achado LOW esperado no primeiro deploy: sessões com token emitido antes do hook existir caem em "pendente" até renovar (no máximo 10 min agora), não é bug.
